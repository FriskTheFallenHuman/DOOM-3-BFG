...
idCVar	fs_resourceLoadPriority( "fs_resourceLoadPriority", "0", CVAR_SYSTEM | CVAR_NOCHEAT, "if 1, open requests will be honored from resource files first; if 0, the resource files are checked after normal search paths" );
...
idFileHandle idFileSystemLocal::OpenOSFile( const char *fileName, fsMode_t mode ) {
	return Sys_OpenOSFile( fileName, mode );
}
...
bool idFileSystemLocal::RenameFile( const char * relativePath, const char * newName, const char * basePath ) {
	...
	return Sys_RenameFile( oldOSPath.c_str(), newOSPath.c_str() );;
}
