...
static void PopulateExceptionView( HWND hList ) {
	...
	lvc.cx   = 140; lvc.pszText = "Property"; ListView_InsertColumn( hList, 0, &lvc );
	lvc.cx   = 200; lvc.pszText = "Value";    ListView_InsertColumn( hList, 1, &lvc );
...
INT_PTR CALLBACK CrashHandlerDialogProc( HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam ) {
	...
	case WM_INITDIALOG: {
			...
			LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES | LVS_EX_AUTOSIZECOLUMNS );
		{
			LVCOLUMN lvc = {};
			lvc.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_SUBITEM;
			lvc.cx = 240; lvc.pszText = "Function";   ListView_InsertColumn( hStackList, 0, &lvc );
			lvc.cx =  50; lvc.pszText = "Line";        ListView_InsertColumn( hStackList, 1, &lvc );
			lvc.cx = 200; lvc.pszText = "Source File"; ListView_InsertColumn( hStackList, 2, &lvc );
			lvc.cx = 150; lvc.pszText = "Module";       ListView_InsertColumn( hStackList, 3, &lvc );
		}
		...
		if ( g_crashStackLen > 0 ) {
			...
			if ( frames.Num() > 0 ) { frames.RemoveIndex( 0 ); }
			if ( frames.Num() > 0 ) { frames.RemoveIndex( 0 ); }
		}
	...
	case WM_MOUSEWHEEL: {
		...
		if ( hFocused ) { SendMessage( hFocused, WM_MOUSEWHEEL, wParam, lParam ); }
		return TRUE;
	}
