...
public:
	...
	virtual void			WriteImage( idImageType filetype, const char *filename, const byte *data, int bytesPerPixel, int width, int height, bool flipVertical = false, const char *basePath = "fs_savepath" ) = 0;
	virtual void			LoadImage( const char *name, byte **pic, int *width, int *height, ID_TIME_T *timestamp ) = 0;
};
